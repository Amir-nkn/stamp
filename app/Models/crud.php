...
abstract class CRUD extends \PDO {
    ...
    // Nom de la table sur laquelle les opérations CRUD seront effectuées
    protected $table;

    // Nom de la clé primaire de la table
    protected $primaryKey = 'id';

    // Champs autorisés à être utilisés lors de l'insertion ou de la mise à jour
    protected $fillable = [];

    // Constructeur qui initialise la connexion PDO à la base de données
    ...
    // Récupère des enregistrements avec tri, limite et décalage (pagination)
    ...
    // Récupère tous les timbres avec jointures sur les tables pays, couleurs, conditions
...
final public function selectIdjoin($id){
    ...
    if ($user_id) {
        $sql .= " LEFT JOIN bids ON bids.stamp_id = stamps.id AND bids.user_id = :user_id";
    }
