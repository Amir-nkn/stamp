...
<head>
    ...
    <title>Éditer le Timbre - Enchères</title>
...
<body>
    ...
    <div class="stamp-form">
        <form action="{{ BASE }}/stamp/edit?id={{ stamp.id }}" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="id" value="{{ stamp.id }}">
            <!-- ID du timbre -->
            <label for="stamp_id">ID du timbre:</label>
            <input type="text" id="stamp_id" name="stamp_id" value="{{ stamp.stamp_id }}" required>
            {% if errors['stamp_id'] %}
                <div class="error">{{ errors['stamp_id'] }}</div>
            {% endif %}
            ...
            <button class="stamp-btn" type="submit">Mettre à jour</button>
     </form>

        <form action="{{ BASE }}/stamp/delete" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce timbre ?');" style="margin-top: 10px;">
            <input type="hidden" name="id" value="{{ stamp.id }}">
            <button type="submit"  class="stamp-delete">Supprimer</button>
        </form>
